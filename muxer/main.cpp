...
{
    const char help[] = "\n\
tsMuxeR is a simple program to mux video to TS/M2TS files or create Blu-Ray discs.\n\
...
\n\
tsMuxeR can be run in track detection mode or in muxing mode. If tsMuxeR is run with\n\
only one argument then tsMuxeR displays input track information required to\n\
construct meta file. If tsMuxeR is run with two arguments tsMuxeR starts muxing\n\
or demuxing process.\n\
\n\
META file format:\n\
The file must have the extension META.\n\
The META file defines the files that you want to multiplex.\n\
The first line of the .meta file contains additional parameters that apply to all tracks.\n\
In this case the line should begin with the word MUXOPT.\n\
\n\
The next lines indicate a list of tracks and their parameters.\n\
The format is as follows: <code name>, <file name>, <parameters>.\n\
Parameters are comma-separated. Each parameter indicates name and value.\n\
...
to a BD structure. The input file name can reference to an elementary stream or\n\
a track inside a container.\n\
\n\
...
\n\
Each track may have additional parameters. Track parameters do not use dash.\n\
If the parameter value has several words, it must be enclosed in quotes.\n\
\n\
...
- down-to-dts       Available only for DTS-HD tracks. Filters out the HD part.\n\
- down-to-ac3       Available only for TRUE-HD tracks. Filters out the HD part.\n\
- secondary         Mux as secondary audio. Available for DD+ and DTS-Express.\n\
\n\
Additional parameters for video tracks:\n\
- fps               Video fps. If not defined, default value is auto-detected from\n\
                    a source stream if present. If not, default value is 23.976.\n\
- delPulldown       Removes pulldown from the track if it exists.  This option leads\n\
                    to fps change from 30 to 24 if pulldown exists.\n\
- ar                Overrides video aspect ratio (16:9, 4:3, etc).\n\
\n\
Additional parameters for H.264 video tracks:\n\
- level             Overwrites level in the H264 stream. NOTE: this option updates\n\
...
- insertSEI         If the original stream does not contain SEI picture timing,\n\
                    SEI buffering period or VUI parameters, then these data are added\n\
                    to the stream. This option is recommended for BD muxing.\n\
- forceSEI          Adds SEI picture timing, buffering period and VUI parameters\n\
                    to the stream. Rebuilds data if data already exist.\n\
- contSPS           If the original video doesn't contain repetitive SPS/PPS then\n\
                    SPS/PPS will be added to the stream before each keyframe.\n\
...
- subTrack          Used  for combined AVC/MVC tracks only. TsMuxeR always\n\
                    ...
                    This parameter defines reference to AVC part(if value=1) or\n\
...
- 3d-plane          Parameter defines number of the '3D offset track' which is\n\
                    placed inside the MVC track. Each message has individual 3D\n\
...
- font-color        Font color. It can be defined in hexadecimal or decimal\n\
                    format. If the value is 24-bit long (for instance 0xFF00FF)\n\
                    it defines RGB components. If the value is 32-bit long\n\
                    (e.g. 0x80FF00FF) it defines ARGB components.\n\
...
- line-spacing      Interval between lines. Default value is 1.0.\n\
\n\
tsMuxeR supports additional tags in the SRT track. The syntax and parameters\n\
are the same used with HTML: <b>, <i>, <u>, <strike>, <font>.\n\
...
\n\
Global additional parameters placed in the first line of the META file (MUXOPT).\n\
...
--minbitrate        Sets the lower limit for the bitrate.  If the stream has\n\
                    a smaller bitrate then it will padded with NULL packets.\n\
--maxbitrate        The upper limit for the bitrate.\n\
--cbr               Muxing mode with fixed bitrate. Options --vbr and --cbr\n\
                    should not be used together.\n\
--vbv-len           The length of the virtual buffer in milliseconds. The\n\
                    default value is 500. Typically this option is used\n\
                    together with --cbr. The parameter is similar to the value of\n\
                    vbv-buffer-size in the x264 encoder, but is defined in\n\
                    milliseconds instead of kbit.\n\
--no-asyncio        Do not create a separate thread for writing. Also, this option\n\
                    disables the flag FILE_FLAG_NO_BUFFERING for writing.\n\
                    Deprecated option.\n\
--auto-chapters     Inserts a chapter every <nn> minutes. Used only in BD/AVCHD mode.\n\
--custom-chapters   A semicolon-delimited list of strings in format hh:mm:ss.zzz.\n\
--demux             In this mode the selected audio and video tracks are stored as\n\
                    separate files instead of muxing. The output name must be a folder\n\
                    name. All selected effects (such as change of level for H.264)\n\
                    are processed. When demuxed, certain types of tracks\n\
                    always get changed when extracted:\n\
                    - Subtitles in a Presentation Graphic Stream are converted to\n\
                      .SUP format.\n\
                    - PCM audio is saved as a .WAV file.\n\
--blu-ray           Mux to a BD structure. If the output name means a folder, a BD structure\n\
                    will be created. For BD3D discs ssif files are not created.\n\
                    If the output name has the .iso extension the BD structure is\n\
                    stored in a single file.\n\
--avchd             Mux to an AVCHD structure.\n\
--cut-start         Trim the beginning of the program. The value should be completed with\n\
                    \"ms\" (the number of milliseconds), \"s\" (seconds) or \"min\" (minutes).\n\
--cut-end           Trim the end of the program. The value should be completed with\n\
                    \"ms\" (the number of milliseconds), \"s\" (seconds) or \"min\" (minutes).\n\
--split-duration    Split the output into several segments. Time is specified in seconds.\n\
--split-size        Split the output into several segments. The values must be written\n\
                    by using one of the following suffixes: Kb,kib, mb,mib, gb,gib.\n\
--right-eye         Use base video stream for the right eye. Used for 3DBD only.\n\
--start-time        Timestamp of the first video frame. Can be defined as 45kHz\n\
                    clock (just a number) or as time in format hh:mm:ss.zzz.\n\
--mplsOffset        The number of the first MPLS file. Used for Blu-Ray mode.\n\
--m2tsOffset        The number of the first M2TS file. Used for Blu-Ray mode.\n\
--insertBlankPL     Adds extra short playlist. Used for cropped video muxed to\n\
                    Blu-Ray disc.\n\
--blankOffset       Blank playlist number.\n\
--label             Volume name of the .ISO file.\n\
--extra-iso-space   Allocate extra space in 64K units for ISO disc metadata\n\
                    (file and directory names). Normally tsMuxeR allocates this\n\
                    space automatically. But if the split ommand generates a lot of\n\
                    small files, then additional ISO space may be required.\n\
";
